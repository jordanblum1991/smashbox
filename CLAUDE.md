...
**RESOLVED: daily reconciliation buckets by shop-local (Seller Center) day and ties settled months to $0.00/day.** Our `Order.placed_at` is the exact `Created Time` from the orders CSV (we DELIBERATELY do not shift stored timestamps — a shift would risk breaking settlement reconciliation, P&L date filters, and policy-violation flagging that all key off `placed_at`). Instead, the daily/GMV reconciliation converts `placed_at` to the shop's DST-aware reporting zone per-row (`placed_local_date` in `app/services/reporting_tz.py`, the UTC-6→America/Los_Angeles model). An offset sweep (2026-05-19; re-verified 2026-06-16 on prod) confirmed this collapses settled-month daily variance to **$0.00/day** — Jan–May 2026 each tie to TikTok's Shop Analytics to the cent with zero off-days. (This supersedes the earlier "exactly 1h offset, ~$7.80 March residual, amber rows are informational" note, which described the pre-bucketing state.) Consequence: on the Reconciliation page's daily block, an amber row on a **settled** day is now a **real break** (missed/partial import or TikTok-side restatement) — `recent_recon_breaks` alerts on it past a `settle_grace_days` window. Amber on the **current/most-recent** days is expected: TikTok's Shop Analytics aggregate lags its own order ledger by a few days and clears as it finalizes (e.g. June 2026 showed a provisional ~$942 gap mid-month).
...
Schema is managed by **Alembic** (`alembic upgrade head`) — `alembic/versions/07c4ee33b7fa_baseline_schema.py` is the baseline (all 27 tables); `tests/test_migrations.py` guards models↔migrations parity. `fly.toml`'s `[deploy] release_command = "alembic upgrade head"` applies pending migrations automatically before each release goes live (a failed migration aborts the deploy). The prod Postgres was **stamped at the baseline** once after the cutover so release-time upgrades only run *new* revisions rather than re-creating existing tables. Boot still calls `Base.metadata.create_all` (idempotent, `checkfirst`) as a belt-and-suspenders for fresh DBs, but **new model columns must now go through an Alembic revision**, not the legacy `_ensure_columns` SQLite shim in `app/main.py` (that shim only fires for missing columns and is inert on the migrated Postgres). Rollback path: prod's pre-cutover SQLite file remains at `/data/smashbox.db` — unset the `DATABASE_URL` secret to revert.
...
### Live SAP inventory feed (no CSV)

On-hand inventory comes from the **SAP feed** (`settings.sap_inventory_url`) rather than a manual CSV upload. `app/services/inventory_sync.py` fetches the JSON (`{Itemcode, WhsCode, OnHand, InventoryDate}` rows), keeps only the **SB** (sellable) warehouse — `SBS` is sample stock tracked separately, `MIA` is "missing inventory", `01` is unused — and feeds the rows through the *same* path as the CSV importer (`inventory_snapshot.import_dataframe`), recorded as an `INVENTORY_SNAPSHOT` `ImportBatch`. `captured_at` truncates to the feed date, so a same-day re-sync is idempotent (upsert in place). The same sync also imports the **SBS** warehouse (sample pool) into a SEPARATE `SampleInventorySnapshot` table — sample and sellable inventory never share a table — which the **sample-inventory report** (`app/reports/sample_inventory.py`) reads as authoritative on-hand (it previously read the empty `SampleInventoryMovement` ledger). One SAP sync writes both tables under one `ImportBatch`. Triggered two ways: a **manual button** on the Uploads page (`POST /uploads/sync-inventory-sap`) and an **in-process APScheduler** weekday cron (`app/services/scheduler.py`) whose time/days/on-off are user-editable on the Uploads page (persisted on `Shop.inventory_sync_*`, fires in `Shop.timezone`). The scheduler runs only when `SCHEDULER_ENABLED=true` (off in dev/tests). The CSV `INVENTORY_SNAPSHOT` importer still works as a fallback.
