...
On-hand inventory comes from the **SAP feed** (`settings.sap_inventory_url`) rather than a manual CSV upload. `app/services/inventory_sync.py` fetches the JSON (`{Itemcode, WhsCode, OnHand, InventoryDate}` rows), keeps only the **SB** (sellable) warehouse — `SBS` is sample stock tracked separately, `MIA` is "missing inventory", `01` is unused — and feeds the rows through the *same* path as the CSV importer (`inventory_snapshot.import_dataframe`), recorded as an `INVENTORY_SNAPSHOT` `ImportBatch`. `captured_at` truncates to the feed date, so a same-day re-sync is idempotent (upsert in place). Triggered two ways: a **manual button** on the Uploads page (`POST /uploads/sync-inventory-sap`) and an **in-process APScheduler** weekday cron (`app/services/scheduler.py`) whose time/days/on-off are user-editable on the Uploads page (persisted on `Shop.inventory_sync_*`, fires in `Shop.timezone`). The scheduler runs only when `SCHEDULER_ENABLED=true` (off in dev/tests). The CSV `INVENTORY_SNAPSHOT` importer still works as a fallback. (SBS sample on-hand is available in the feed but not yet wired into the sample-inventory report — that's the deferred follow-up.)
